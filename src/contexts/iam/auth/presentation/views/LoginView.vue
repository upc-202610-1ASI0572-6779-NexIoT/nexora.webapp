<template>
  <div class="login-page">
    <div v-if="showPendingOverlay" class="pending-overlay" @click.self="closePendingModal">
      <div class="pending-modal">
        <div class="pending-modal__icon">
          <font-awesome-icon :icon="subscriptionStatus === 'Expired' || subscriptionStatus === 'Suspended' ? 'triangle-exclamation' : 'circle-exclamation'" />
        </div>
        <h2 class="pending-modal__title">
          {{ subscriptionStatus === 'Expired' ? 'Suscripción Expirada' :
             subscriptionStatus === 'Suspended' ? 'Suscripción Suspendida' :
             'Suscripción Pendiente' }}
        </h2>
        <p class="pending-modal__text">
          <template v-if="subscriptionStatus === 'Expired'">
            Tu suscripción ha expirado. Renueva tu plan para seguir accediendo a la plataforma.
          </template>
          <template v-else-if="subscriptionStatus === 'Suspended'">
            Tu suscripción ha sido suspendida. Contacta con soporte para regularizar tu situación.
          </template>
          <template v-else>
            Hemos detectado que tu cuenta aún no ha completado el proceso de activación de la suscripción.
            Debes finalizarlo para acceder a la plataforma.
          </template>
        </p>
        <div class="pending-modal__actions">
          <button
            v-if="subscriptionStatus !== 'Suspended'"
            class="pending-modal__btn pending-modal__btn--primary"
            type="button"
            @click="continueActivation"
          >
            {{ subscriptionStatus === 'Expired' ? 'Renovar Plan' : 'Continuar Activación' }}
            <font-awesome-icon icon="arrow-right" />
          </button>
          <button class="pending-modal__btn pending-modal__btn--secondary" type="button" @click="closePendingModal">
            Cerrar Sesión
          </button>
        </div>
      </div>
      ...
      <main class="login-main">
        <div class="login-main__body">
          ...
          <footer class="login-main__footer">
            <p>
              New to Nexora?
              <router-link to="/register" class="login-main__link">Create your account</router-link>
            </p>
          </footer>
        </div>
      </main>
...
const handleLogin = async (credentials) => {
  try {
    ...
    if (status && status.toLowerCase() === 'active') {
      ...
      subscriptionStatus.value = status || null;
...
const continueActivation = () => {
  showPendingOverlay.value = false;
...
.pending-modal__btn--primary {
  background: var(--primary-color);
  color: #ffffff;
}

.pending-modal__btn--primary:hover {
  background: #e66700;
  box-shadow: 0 4px 15px rgba(255, 115, 0, 0.3);
  transform: translateY(-1px);
}

.pending-modal__btn--secondary {
  background: transparent;
  color: var(--secondary-color);
  border: 2px solid var(--secondary-color);
}

.pending-modal__btn--secondary:hover {
